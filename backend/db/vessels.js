...
/**
 * Get a vessel by IMO for a tenant
 * SECURITY: Verifies vessel belongs to the specified tenant
 */
export async function getVesselByImo(imo, tenantId) {
  validateTenantId(tenantId, 'getVesselByImo');
  ...
  try {
    // CRITICAL: Always filter by both imo AND tenant_id to prevent cross-tenant access
    const result = await query(
      'SELECT * FROM vessels WHERE imo = $1 AND tenant_id = $2',
      [String(imo), tenantId]
    );
    if (result.rows.length === 0) return null;
    const row = result.rows[0];
    ...
    console.error('Error fetching vessel by IMO from database:', error);
    // Fallback to mock data
    const { getMockVessels } = await import('../data/mockData.js');
    const vessels = getMockVessels(tenantId);
    return vessels.find(v => v.imo === String(imo)) || null;
  }
}
