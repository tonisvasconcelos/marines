/**
 * MyShipTracking API Error Code Mapping
 * Maps MyShipTracking error codes to standardized error responses
 */

/**
 * Error code mapping for MyShipTracking API errors
 * Maps API error codes to internal error types
 */
export const ERROR_CODE_MAP = {
  ...
  'ERR_INTERNAL': {
    ...
    description: 'An internal error occurred in the MyShipTracking API. Please try again later.',
  },
  'ERR_NOT_FOUND': {
    status: 404,
    userFacing: true,
...
/**
 * Create a standardized error from MyShipTracking API error
 * @param {Object} errorData - Error data from API response
 * @param {number} httpStatus - HTTP status code
 * @param {string} endpoint - API endpoint that failed
 * @returns {Object} Standardized error object
 */
export function createMyShipTrackingError(errorData, httpStatus, endpoint) {
  ...
  // Get error mapping
  const errorMapping = ERROR_CODE_MAP[errorCode] || ERROR_CODE_MAP['ERR_INTERNAL'];
